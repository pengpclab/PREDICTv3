...
#' @param er The patient's estrogen receptor status (positive or negative).
#' @param pr The patient's progesterone receptor status (positive or negative).
#' @param her2 The status of the patient's human epidermal growth factor receptor 2 (HER2).
#' @param ki67 The status of the patient's Ki-67 protein, a marker of cell proliferation.
#' @param size The size of the patient's tumor, usually measured in centimeters.
#' @param grade The grade of the tumor, indicating its aggressiveness.
#' @param screen The method used for disease detection (e.g., screening or symptomatic).
#' @param nodes The status of lymph nodes (e.g., number of positive nodes).
#' @param radio Indicates whether the patient has received radiotherapy.
#' @param heart.gy The radiation dose (measured in Gray) delivered to the heart during radiotherapy.
#' @param horm Indicates whether the patient has received hormone therapy.
#' @param gen The generation of radiotherapy received by the patient (e.g., first, second).
#' @param traz Indicates whether the patient has received trastuzumab.
#' @param bis Indicates whether the patient has received bisphosphonates.
#' @param censor The number of years to consider since surgery (e.g., 5, 10, or 15 years).
#'
#' @export
#'
#' @examples
...
PREDICTv3 <- function(year, age, smoker, er, pr, her2, ki67,
                       ...
                       radio, heart.gy, horm, gen, traz, bis, censor) {

  coeffs <- get0("coeffs", envir = asNamespace("PREDICTv3"))

  ag1_er0 <- as.numeric(unlist(coeffs[1,2]))
  ...
  censor <- censor
  if (is.wholenumber(censor)) { time <- 1:censor } else {time <- c(1:floor(censor), censor)}
  ...
  r.other  <- 1.02 # Relative hazard other disease mortality per Gy heart dose from Taylor et al (2017)
  # RR 1.15 for average heart dose of 6 Gy.
  ...
  c.oth <- ifelse(gen==0, 0, log(c.other)) # winton fix, for easier comparisons with cljs code
  ...
  c     <- ifelse(gen== 0, 0, ifelse(gen== 2, -.248, -.446))
  h     <- ifelse(horm==1 & er==1, -0.3857, 0)
  r     <- ifelse(radio==1, log(r.breast), 0)
  t     <- ifelse(her2==1 & traz==1, -.3567, 0)
  b     <- ifelse(bis==1, -0.198, 0)

  # Treatment combined
  rx <- tibble(s=rep(0, length(time)), rx=rep(r+h+c+t+b, length(time)))
  pi.rx <- rx + pi
  mi.rx <- tibble(s = rep(mi, length(time)),
                  mi.rx = rep(mi + c.oth + r.oth, length(time))) #
  ...
  base.m.cum.br <- if (er==1) {
    exp(h1_br_i + h1_br_t1*((time/10)^-0.5) + h1_br_t2*((time/10)^-0.5*log(time/10)))
  ...
  return(as.numeric(surv[length(time), 2]))
}
